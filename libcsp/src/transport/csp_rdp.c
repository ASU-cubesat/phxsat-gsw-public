...
static int csp_rdp_send_cmp(csp_conn_t * conn, csp_packet_t * packet, int flags, int seq_nr, int ack_nr) {
    ...
    csp_log_protocol("RDP: Sent in S %u: syn %u, ack %u, eack %u, "
	...
	csp_log_protocol("RDP: idout.pri=%d,idout.dst=%d,idout.dport=%d,idout.src=%d,idout.sport=%d,idout.flags=%d,idout.ext=%d",idout.pri,idout.dst,idout.dport,idout.src,idout.sport,idout.flags,idout.ext);
