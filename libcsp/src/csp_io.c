...
#ifdef CSP_USE_CRC32
			idout.flags = 0; // Remove CRC flag from idout
			/* Calculate and add CRC32 (does not include header for backwards compatability with csp1.x) */
			//printf("Flags set to %d! Appending CRC...\n",idout.flags);
			if (csp_crc32_append(packet, true) != 0) {  //Modified to true to include header
				/* CRC32 append failed */
...
int csp_sendto(uint8_t prio, uint8_t dest, uint8_t dport, uint8_t src_port, uint32_t opts, csp_packet_t * packet, uint32_t timeout) {
	...
	if (opts & CSP_O_CRC32) {
		...
		printf("csp_sendto: CRC used, flags set to %0x\n", packet->id.flags);
#else
